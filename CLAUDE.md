...
> Заполнено по утверждённой `ARCHITECTURE.md` 0.4 (PRODUCT.md 1.2). Команды ниже фактические — проверены выполнением при TASK-001 (инициализация монорепозитория); детали и обоснование решений — ARCHITECTURE 4 и 4.1.
...
- **Структура репозитория:** монорепозиторий pnpm workspaces + Turborepo (ARCHITECTURE 4): `apps/api` (NestJS — HTTP-процесс `src/main.ts` и worker-процесс `src/worker.ts`), `apps/mobile` (Expo; пока автономен, пакеты монорепозитория не использует), `apps/supplier-web` (React + Vite), `apps/admin-web` (React + Vite); `packages/contracts` (zod-схема, пока одна демонстрационная — `healthCheckResponseSchema`), `packages/domain` (чистая доменная логика — пока `normalizeArticle` с тестом), `packages/i18n` (kk/ru/en, пока один ключ), `packages/ui` (веб-компоненты, пока `Button`), `packages/dynamic-forms` (каркас, без функциональности), `packages/config` (общие `tsconfig`/`eslint`); `infra/docker` (compose для dev; `infra/migrations` появится в TASK-002). `PRODUCT.md`, `ARCHITECTURE.md`, `PROJECT_PLAN.md`, `PROJECT_STATE.md`, `tasks/` — в корне репозитория, не в `docs/`. Модель данных, реальные эндпоинты и полный контракт клиент–сервер — TASK-002/003.
- **Запуск dev:** `docker compose -f infra/docker/compose.dev.yml up -d` (PostgreSQL, Redis, Meilisearch, MinIO; моков внешних сервисов пока нет — появятся с интеграциями), затем `pnpm install` и `pnpm dev` (Turborepo: `apps/api` на `:3000` с `/health`, `apps/admin-web` на `:5174`, `apps/supplier-web` на `:5175`); worker — отдельно и отдельной командой, `pnpm --filter api worker` (dev) или после `pnpm build` — `pnpm --filter api start:worker`; мобильное — `pnpm --filter mobile start` (Expo dev server; запуск на реальном эмуляторе/симуляторе в этой задаче не проверялся — на машине разработки нет Android SDK, см. TASK-001-REPORT).
- **Тесты:** `pnpm test` — unit (Vitest), одной командой для всего репозитория через Turborepo; сейчас покрывает `packages/domain`, `packages/contracts`, `packages/i18n`, `apps/api`. `pnpm test:integration` (Testcontainers), `pnpm test:contract` (OpenAPI), E2E (Playwright/Maestro), набор качества ИИ — скриптов ещё нет, появятся вместе с соответствующей функциональностью (TASK-002+).
- **Lint / typecheck / build:** `pnpm format` / `pnpm format:check` (Prettier), `pnpm lint` (ESLint 10 + typescript-eslint, общая конфигурация `packages/config/eslint`; запрещает импорт внутренностей чужого пакета в обход его `package.json.exports` — ARCHITECTURE 4.1 I4; правило про прямые вызовы SDK мониторинга появится вместе с модулем `observability`), `pnpm typecheck` (tsc по workspace, конфиги — `packages/config/typescript`), `pnpm build` (Turborepo; собирает пакеты, `apps/api` и веб-приложения; `apps/mobile` — нативная сборка вне CI по этой задаче, EAS Build — позже).
- **Миграции:** появятся в TASK-002 вместе с моделью данных (Drizzle, SQL-миграции в `infra/migrations`, команды `pnpm --filter api migrate` / `migrate:generate`, обратимость, проверка на копии prod-снимка на staging). В TASK-001 модели данных и миграций нет.
