...
export interface MoveRequest {
  ...
  channel: Extract<OrderEventChannel, "app" | "supplier_web">;
...
export type DeadlineOutcome = DeadlineAction | "reserve_warned" | null;
...
export function dueDeadline(row: OrderRow, at: Date): DeadlineAction | "warn" | null {
...
export class OrderTransitions {
  ...
  async move(tx: DbExecutor, request: MoveRequest): Promise<MoveOutcome> {
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
      ...
      if (stale || orderTransition(row.status, request.action) === null) {
        return this.refuse(tx, row, request, at);
      }
      const moved = await this.apply(tx, row, request.action, request.actor, request.channel, at, {
    ...
    return this.refuse(tx, await this.read(tx, request.orderId), request, new Date());
  }
  ...
  /** The last move of an order (not a note), for a conflict's answer. */
  async lastMove(executor: DbExecutor, orderId: string): Promise<OrderEventRow | null> {
    const [event] = await executor
      .select()
      .from(orderEvent)
      .where(and(eq(orderEvent.orderId, orderId), isNotNull(orderEvent.toStatus)))
  ...
  private async refuse(
    ...
    if (request.actor.type === "supplier_member") {
      ...
      // press is kept against the employee who made it.
      await this.record(
        tx,
        row,
        "late_action_ignored",
  ...
  private async apply(
    ...
    at: Date,
    extra: { decline?: MoveRequest["decline"] } = {},
    ...
    switch (action) {
      ...
      case "expire_reserve":
        ...
        break;
      case "close":
        throw new Error("Closing by the code comes with TASK-022");
    }
   ...
   * once it has. Checked in the update itself, so a deadline reached between
   * the read and the write is never jumped over.
   */
