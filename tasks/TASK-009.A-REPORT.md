...
12. `fd3488b` Update TASK-009.A report with the second CI run and the sweeper test fix — `tasks/TASK-009.A-REPORT.md`.
13. `473208e` Keep the scheduled test sweeper out of the job tests — `apps/api/src/jobs/jobs.integration.test.ts`.
14. Коммит с обновлённым отчётом — `tasks/TASK-009.A-REPORT.md` (хэш — в ответе сессии; в самом файле его быть не может).
...
- Run **35314700700** (коммит `fd3488b`) — **failure**: третий тест того же файла — «says nothing about a periodic run that had nothing to do, and sums them up» — `Timed out waiting for: the quiet runs to be summed up` (203/204). Общая причина всех трёх падений: в блоке «with two workers» тестовый свипер `test.sweep` запланирован раз в минуту (свипер по умолчанию), и плановый прогон, попавший внутрь теста, забирал строки или добавлял свой «тихий» запуск в сводку. В TASK-008/009 граница минуты, видимо, редко попадала внутрь этих тестов; новые интеграционные сценарии TASK-009.A (worker и отдельный процесс в `observability.integration.test.ts`, идущие параллельно) сдвинули тайминги CI. Исправлено в корне в `473208e`: тестовому свиперу задано расписание раз в год (тесты запускают его сами — `runNow`, `SweepRunner.run`); код приложения не менялся. Локально файл — 21/21 PASS.
...
- **CI run 35314700700 — третья гонка того же происхождения** (лишний «тихий» плановый запуск `test.sweep` в сводке). Корень устранён: тестовому свиперу задано расписание раз в год (`473208e`); см. «CI».
- **Правка файла через Python исказила регулярные выражения** (`\b` стал символом backspace, `\0` — литеральным NUL). Найдено по тому, что правило учётных данных URL не срабатывало; исправлено, все изменённые файлы проверены на управляющие символы; маркер удержания теперь строится в коде (`String.fromCharCode(0)`).
